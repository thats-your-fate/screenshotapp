...
export default async function EditTemplateMetaPage({
  params,
}: {
    ...
    <div className="max-w-2xl space-y-4 rounded-xl border border-slate-200 bg-white p-6">
      ...
      <form action={updateTemplateMetaAction.bind(null, templateId)} className="grid gap-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          <input name="canvasWidth" type="number" defaultValue={template.canvasWidth} required className="rounded-md border border-slate-300 px-3 py-2 text-sm" />
          <input name="canvasHeight" type="number" defaultValue={template.canvasHeight} required className="rounded-md border border-slate-300 px-3 py-2 text-sm" />
        </div>
