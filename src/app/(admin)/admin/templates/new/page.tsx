    ...
    <div className="max-w-2xl space-y-4 rounded-xl border border-slate-200 bg-white p-6">
      ...
      <form action={createTemplateAction} className="grid gap-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          <input name="canvasWidth" type="number" defaultValue={1242} required className="rounded-md border border-slate-300 px-3 py-2 text-sm" />
          <input name="canvasHeight" type="number" defaultValue={2688} required className="rounded-md border border-slate-300 px-3 py-2 text-sm" />
        </div>
        ...
        <button type="submit" className="rounded-md bg-slate-900 px-3 py-2 text-sm font-semibold text-white">Create</button>
      </form>
    </div>
  );
}
