...
function getMarketingHomePath(locale: LandingLocale) {
  return locale === "en" ? "/" : `/${locale}/`;
}

function AstraxLogo({ dark = false, locale = "en" }: { dark?: boolean; locale?: LandingLocale }) {
  return (
    <Link className="navbar-brand py-5" href={getMarketingHomePath(locale)}>
        ...
        {languageOptions.map((item) => (
          <li key={item.locale}>
            <Link
              ...
              href={item.href}
              onClick={() => {
                document.cookie = `app_locale=${item.locale}; Path=/; Max-Age=31536000; SameSite=Lax`;
                setOpen(false);
              }}
            >
              <span>{item.flag}</span>
      ...
      <div className="position-absolute top-0 start-0 w-100">
        <nav className="navbar navbar-expand-lg navbar-transparent z-5 p-0 shadow-none">
          <div className="container">
            <AstraxLogo locale={locale} />
            <div className="d-none d-lg-flex align-self-stretch z-35 position-relative">
              <ul className="navbar-nav mx-auto gap-4 align-items-lg-center">
                {navLinks.map((link) => (
                  <li key={link.href} className="nav-item">
                    <Link className="nav-link text-uppercase" href={link.href} onClick={link.href === "/sign-in" ? rememberLocale : undefined}>
                      {link.label}
                    </Link>
                  </li>
                ))}
            ...
            <div className="d-flex align-items-center gap-4 align-self-stretch">
              <Link href="/sign-up" className="btn btn-dashed d-none d-md-flex" onClick={rememberLocale}>
                {copy.nav.startFree}
              ...
              <div className="rounded-4 border border-white border-opacity-25 bg-dark p-4">
                <div className="d-flex flex-column gap-3">
                  {navLinks.map((link) => (
                    <Link
                      key={link.href}
                      className="btn-text text-white text-uppercase"
                      href={link.href}
                      onClick={() => {
                        if (link.href === "/sign-in") rememberLocale();
                        setMobileOpen(false);
                      }}
                    >
                      {link.label}
